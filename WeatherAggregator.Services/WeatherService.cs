...
namespace WeatherAggregator.Services
{
    public class WeatherService : IWeatherService
    {
        ...
        {
            var repository = this.ResolveWeatherRepository(this.SelectRepositorySet(resourceName));
            return repository.GetWeatherData(cityModel);
        }

        private IWeatherRepository ResolveWeatherRepository(RepositorySet repository)
        {
            var lazy = this.repositorySetMeta.FirstOrDefault(e => e.Metadata.RepositorySet == repository);
            if (lazy == null)
            {
